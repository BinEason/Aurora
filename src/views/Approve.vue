      ...
      <el-table :data="applyList">
        ...
        <el-table-column label="操作" width="150">
          <template #default="scope">
            <el-button size="default" @click="handleDetail(scope.row)" v-if="
...
import { getApplyListAPI, leaveApproveAPI } from '../api'
      ...
      ctx.$refs.dialogForm.validate(async (valid) => {
        if (valid) {
          ...
          try {
            ...
            getApplyList();
            store.commit(
              "m_user/setNoticeCount",
              store.state.m_user.userInfo.noticeCount - 1
            );
            console.log(store.state.m_user.userInfo.noticeCount);
          } catch (error) { }
        }
      });
    };
