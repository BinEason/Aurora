            ...
            <el-table :data="menuList" row-key="_id" :tree-props="{ children: 'children' }">
                ...
                <el-table-column label="操作" width="250">
                    <template #default="scope">
                        <el-button @click="handleAdd(2, scope.row)" type="primary" size="default" v-has="'menu-create'">
                            新增</el-button>
                        <el-button @click="handleEdit(scope.row)" size="default" v-has="'menu-edit'">编辑</el-button>
                        <el-button type="danger" size="default" @click="handleDel(scope.row._id)" v-has="'menu-delete'">
                            删除</el-button>
                    </template>
                </el-table-column>
            </el-table>
        </div>
        <el-dialog :title="title" v-model="showModal" :show-close="false" :close-on-click-modal="false">
            <el-form ref="dialogForm" :model="menuForm" label-width="100px" :rules="rules">
                <el-form-item label="父级菜单" prop="parentId">
                    <el-cascader v-model="menuForm.parentId" :options="menuList" placeholder="父级菜单"
...
import { menuCreateSubmitAPI } from '../api'
...
export default {
    ...
    methods: {
        ...
        async getMenuList() {
            try {
                let list = await getMenuListAPI(this.queryForm);
                this.menuList = list;
            } catch (e) {
                throw new Error(e);
            }
        ...
        async handleDel(_id) {
            await menuCreateSubmitAPI({ _id, action: "delete" });
        ...
        handleSubmit() {
            this.$refs.dialogForm.validate(async (valid) => {
                if (valid) {
                    ...
                    let res = await menuCreateSubmitAPI(params);
                    console.log(res);
