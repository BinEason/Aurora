<template>
  <div class="role-manage">
      ...
      <el-table :data="roleList">
        ...
        <el-table-column label="操作" width="260">
          <template #default="scope">
            <el-button size="default" @click="handleEdit(scope.row)" v-has="'role-edit'">编辑</el-button>
            <el-button size="default" type="primary" @click="handleOpenPermission(scope.row)" v-has="'role-setting'">
              设置权限</el-button>
            <el-button type="danger" size="default" @click="handleDel(scope.row._id)" v-has="'role-delete'">删除
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="用户新增" v-model="showModal" :show-close="false" :close-on-click-modal="false">
...
import { getRoleAPI, roleOperateAPI, updateRoleOperateAPI, getMenuListAPI } from '../api'
...
export default {
      ...
      columns: [
        ...
        },
        {
          ...
          formatter: (value) => {
            console.log(value);
            let names = [];
            let list = value.permissionList.checkedKeys || [];
            // console.log(value.permissionList.halfCheckedKeys);
            list.map((key) => {
              // console.log(key);
  ...
  methods: {
    ...
    async getRoleList() {
      try {
        let { list, page } = await getRoleAPI({
          ...this.queryForm,
          ...this.pager,
        });
        this.roleList = list;
    ...
    handleSubmit() {
      this.$refs.dialogForm.validate(async (valid) => {
        if (valid) {
          ...
          let res = await roleOperateAPI(params);
          if (res) {
            this.showModal = false;
    ...
    async handlePermissionSubmit() {
      ...
      await updateRoleOperateAPI(params);
