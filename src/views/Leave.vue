<template>
  <div class="user-manage">
    ...
    <div class="base-table">
      <div class="action">
        <el-button type="primary" @click="handleApply" v-has="'leave-add'">申请休假</el-button>
      </div>
      <el-table :data="applyList">
        ...
        <el-table-column label="操作" width="180">
          <template #default="scope">
            <el-button size="default" @click="handleDetail(scope.row)">查看</el-button>
            <el-button type="danger" size="default" @click="handleDelete(scope.row._id)"
              v-if="[1, 2].includes(scope.row.applyState)" v-has="'leave-delete'">作废</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="申请休假" v-model="showModal" width="70%" :show-close="false" :close-on-click-modal="false">
...
import { getApplyListAPI, leaveOperateAPI } from '../api'
  ...
  setup() {
    ...
    const columns = reactive([
      ...
      },
      {
        ...
        formatter(row) {
          return (
            dayjs(new Date(row.startTime)).format('YYYY-MM-DD') +
            "到" +
            dayjs(new Date(row.endTime)).format('YYYY-MM-DD')
    ...
    const getApplyList = async () => {
      ...
      let { list, page } = await getApplyListAPI(params);
      ...
      ctx.$refs.dialogForm.validate(async (valid) => {
        if (valid) {
          try {
            ...
            let res = await leaveOperateAPI(params);
    ...
    const handleDetail = (row) => {
      ...
      data.time =
        dayjs(new Date(data.startTime)).format('YYYY-MM-DD') +
        "到" +
        dayjs(new Date(data.endTime)).format('YYYY-MM-DD')
      // 1:待审批 2:审批中 3:审批拒绝 4:审批通过 5:作废
      data.applyStateName = {
        1: "待审批",
        2: "审批中",
